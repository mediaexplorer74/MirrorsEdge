  ...
  public class InWindow
  {
    ...
    public void MoveBlock()
    {
      ...
      for (uint index = 0; index < num2; ++index)
        this._bufferBase[(int) index] = this._bufferBase[(int) (num1 + index)];
    ...
    public uint GetMatchLen(int index, uint distance, uint limit)
    {
      ...
      uint matchLen = 0;
      while (matchLen < limit && (int) this._bufferBase[(int) (num + matchLen)] == (int) this._bufferBase[(int) (num + matchLen - distance)])
        ++matchLen;
      return matchLen;
    }

...
namespace game
{
  public class CollOrthoRampNegX : CollShape
  {
    public CollOrthoRampNegX(MathOrthoBox orthoBounds)
      : base(CollShape.ShapeType.SHAPE_TYPE_RAMP_NEG_X, orthoBounds)
    {
    }

    public CollOrthoRampNegX(float x1, float y1, float z1, float x2, float y2, float z2)
      : base(CollShape.ShapeType.SHAPE_TYPE_RAMP_NEG_X, x1, y1, z1, x2, y2, z2)
    {
    }
    ...
    public override bool intersects(MathLine line, ref float minT, ref float maxT)
    {
      ...
      if (!flag1 || flag2 || !flag3 || !GameCommon.inBounds(this.m_globalOrthoBounds.min.x, line.origin.x, this.m_globalOrthoBounds.max.x) || !GameCommon.inBounds(this.m_globalOrthoBounds.min.z, line.origin.z, this.m_globalOrthoBounds.max.z))
        return false;
      float y = this.m_globalOrthoBounds.max.y + (float) (((double) this.m_globalOrthoBounds.min.y - (double) this.m_globalOrthoBounds.max.y) * ((double) line.origin.x - (double) this.m_globalOrthoBounds.min.x) / ((double) this.m_globalOrthoBounds.max.x - (double) this.m_globalOrthoBounds.min.x));
    ...
    {
      MathVector direction = new MathVector(this.m_globalOrthoBounds.max.y - this.m_globalOrthoBounds.min.y, this.m_globalOrthoBounds.max.x - this.m_globalOrthoBounds.min.x, 0.0f);
      direction.normalise();
      MathVector point = new MathVector(this.m_globalOrthoBounds.min.x, this.m_globalOrthoBounds.max.y, this.m_globalOrthoBounds.min.z);
      float closestTtoPoint = MathLine.calculateClosestTToPoint(direction, point);
      point.x = this.m_globalOrthoBounds.max.x;
      point.y = this.m_globalOrthoBounds.min.y;
      float maxT = Math.Max(closestTtoPoint, MathLine.calculateClosestTToPoint(direction, point));
    ...
    public override void setNonOrthogonalRangeOnAxis(SeperatedAxis axis, int shapeIndex)
    {
      ...
      else if (flag2)
      {
        ...
        {
          MathVector point = new MathVector(this.m_globalOrthoBounds.min.x, this.m_globalOrthoBounds.max.y, z1);
          minT = axis.getDistanceOfPoint(point);
          point.y = this.m_globalOrthoBounds.min.y;
          maxT = axis.getDistanceOfPoint(point);
        }
      ...
      else if ((double) direction.x < -0.0099999997764825821 && 0.0099999997764825821 < (double) direction.y)
      {
        MathVector point = new MathVector(this.m_globalOrthoBounds.max.x, this.m_globalOrthoBounds.min.y, z1);
        minT = axis.getDistanceOfPoint(point);
        point.set(this.m_globalOrthoBounds.min.x, this.m_globalOrthoBounds.max.y, z2);
        maxT = axis.getDistanceOfPoint(point);
      }
      else if (0.0099999997764825821 < (double) direction.x && (double) direction.y < -0.0099999997764825821)
      {
        MathVector point = new MathVector(this.m_globalOrthoBounds.min.x, this.m_globalOrthoBounds.max.y, z1);
        minT = axis.getDistanceOfPoint(point);
        point.set(this.m_globalOrthoBounds.max.x, this.m_globalOrthoBounds.min.y, z2);
        maxT = axis.getDistanceOfPoint(point);
      }
